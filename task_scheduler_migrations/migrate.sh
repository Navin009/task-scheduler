...
# Cleanup Docker volume if it exists
if [ -n "$(docker volume ls -q -f name=postgres_data)" ]; then
    echo "Removing existing Docker volume..."
    docker volume rm postgres_data
fi

# Run cleanup script first using Docker's psql
echo "Running cleanup script..."
docker exec postgres psql -U "${DATABASE_USER}" -d "${DATABASE_NAME}" -f /docker-entrypoint-initdb.d/cleanup.sql
